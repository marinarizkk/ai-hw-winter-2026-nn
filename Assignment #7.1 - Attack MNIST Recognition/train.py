...
class Transformer(nn.Module):
    """Transformer for MNIST - Improved version with GELU and better stability"""
    ...
    def __init__(self):
        ...
        # Smaller initialization for positional embedding (prevents instability)
        ...
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=4)  # 4 layers for better capacity
...
def train_single_model(model, model_name, train_loader, val_loader, device, epochs=15):
    """Train a single model and return training history"""

    # Different settings for Transformer vs other models
    if model_name == 'Transformer':
        # Lower learning rate for transformer
        optimizer = optim.Adam(model.parameters(), lr=0.0005)
        # Cosine annealing works better for transformers
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs)
        use_grad_clipping = True
        print(f"  Using Transformer-specific settings: LR=0.0005, Gradient Clipping, Cosine Annealing")
    else:
        # Standard settings for MLP and CNN
...
def main():
    ...
    for model_name, model in models_config:
        ...
        # Train the model (15 epochs for better convergence)
        start_time = time.time()
        trained_model, train_losses, train_accs, val_losses, val_accs, best_val_acc = train_single_model(
            model, model_name, train_loader, val_loader, device, epochs=15
        )
